...
static FileCopyProgressCallback g_copy_progress_callback = NULL;
static bool g_copy_quiet = false;

void FileCopySetProgressCallback(FileCopyProgressCallback callback){
    g_copy_progress_callback = callback;
}
...
ErrCode_t FileCopy(const char *locin, const char *locout, u8 options){
    ...
    if ((options & COPY_MODE_PRINT) && !g_copy_quiet){
        gfx_printf("[  0%%]");
        x += 16;
    }
    ...
    u32 last_callback_percent = 0;
    if (g_copy_progress_callback)
        g_copy_progress_callback(locin, 0);
    ...
    while (sizeRemaining > 0){
        ...
        u32 copy_percent = totalsize ? (u32)(((totalsize - sizeRemaining) * 100) / totalsize) : 100;

        if ((options & COPY_MODE_PRINT) && !g_copy_quiet){
            gfx_con_setpos(x, y);
            gfx_printf("%3d%%", copy_percent);
        }

        
        if (g_copy_progress_callback &&
            (copy_percent == 100 || copy_percent >= last_callback_percent + 5)) {
            last_callback_percent = copy_percent;
            g_copy_progress_callback(locin, copy_percent);
        }
    ...
    if (g_copy_progress_callback && totalsize == 0)
        g_copy_progress_callback(locin, 100);
