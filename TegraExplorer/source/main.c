...
#include "tegraexplorer/aio_cfw_updater.h"
#include "err.h"
#include <soc/pmc.h>
#include "keys/keys.h"
...
void ipl_main()
{
	...
	if (!h_cfg.errors && aio_cfw_update_pending()) {
		aio_cfw_update_run(h_cfg.t210b01 || h_cfg.rcm_patched);

		EPRINTF("CFW updater returned unexpectedly.");
		sd_end();
		while (true)
			bpmp_halt();
	}
