...
bool AmsTab::CreateDownloadItems(const nlohmann::ordered_json& cfw_links, bool hekate, bool ams)
{
    ...
    if (links.size() && !this->hekate.empty()) {
...
void AmsTab::CreateStagedFrames(const std::string& text, const std::string& url, bool erista, bool ams, bool hekate, const std::string& text_hekate, const std::string& hekate_url)
{
    ...
    stagedFrame->setTitle(this->type == contentType::ams_cfw ?
        "menus/ams_update/getting_ams"_i18n :
        "menus/ams_update/custom_download"_i18n);

    stagedFrame->addStage(
        new ConfirmPage(stagedFrame, text));

    stagedFrame->addStage(
        new WorkerPage(stagedFrame, "menus/common/downloading"_i18n,
            [&, url]() { util::downloadArchive(url, this->type); }));

    stagedFrame->addStage(
        new WorkerPage(stagedFrame, "menus/common/extracting"_i18n,
    ...
    if (hekate) {
        ...
        stagedFrame->addStage(
            new WorkerPage(stagedFrame, "menus/common/downloading"_i18n,
                [hekate_url]() { util::downloadArchive(hekate_url, contentType::bootloaders); }));

        stagedFrame->addStage(
            new WorkerPage(stagedFrame, "menus/common/extracting"_i18n,
                []() { util::extractArchive(contentType::bootloaders); }));
    }
...
void AmsTab_Regular::CreateLists()
{
    ...
    this->addView(new brls::Label(
        ...
        "menus/main/ams_text"_i18n +
            ...
            (erista ? "\n" + "menus/ams_update/erista_rev"_i18n : "\n" + "menus/ams_update/mariko_rev"_i18n),
        true));
    ...
    this->addView(new brls::Label(
        brls::LabelStyle::DESCRIPTION,
        "menus/ams_update/cfw_pack_label"_i18n,
        true));

    listItem = new brls::ListItem("menus/ams_update/cfw_install_pack"_i18n);
    listItem->setHeight(LISTITEM_HEIGHT);
    listItem->getClickEvent()->subscribe([this](brls::View* view) {
        std::string url = "https://github.com/alucardiio-yt/archives-nx/releases/latest/download/NX.latest.zip";
        std::string text = "menus/ams_update/cfw_download_latest"_i18n;

        brls::StagedAppletFrame* stagedFrame = new brls::StagedAppletFrame();
        stagedFrame->setTitle("menus/ams_update/getting_ams"_i18n);
        ...
        stagedFrame->addStage(
            new WorkerPage(stagedFrame, "menus/common/downloading"_i18n,
                [url]() {
                    util::downloadArchive(url, contentType::ams_cfw);
                }));

        stagedFrame->addStage(
            new WorkerPage(stagedFrame, "menus/ams_update/cfw_preparing_pack"_i18n,
                [this]() {
                    util::prepareAndValidateCfwUpdateStaging(this->erista);
                }));

        stagedFrame->addStage(
            new ConfirmPage_AmsUpdate(stagedFrame, "menus/ams_update/cfw_ready_reboot"_i18n, this->erista));
